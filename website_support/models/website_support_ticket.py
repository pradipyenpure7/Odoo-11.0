...
class WebsiteSupportTicket(models.Model):
    ...
    ticket_number = fields.Char(string="Ticket Number", readonly=True,store=True)
    ...
    date_create_new=fields.Date("Create Date",compute='new_date_order',store=True)
    ...
    def new_date_order(self):
        ...
        for dt in self:
            if dt.create_date!=False:
                temp = datetime.datetime.strptime(dt.create_date,"%Y-%m-%d %H:%M:%S").date()
                dt.date_create_new=temp
            else:
                dt.date_create_new=""
    
    def status_mail(self):
        email_template = self.env['ir.model.data'].get_object('website_support','support_ticket_status_mail')
    ...
    def action_assigned(self):
        #self.status_mail()
        return self.write({'status': 'assigned','time_assign':datetime.datetime.now()})
    @api.multi
    ...
    def action_resolved(self):
        #self.status_mail()
        return self.write({'status': 'resolved','time_resolve':datetime.datetime.now()})
    @api.multi
    ...
    def set_ticket_number(self,vals):
        ...
        if sun_tag_ids.name=="Incident":
            print("Incident")
            vals['ticket_number'] = self.env['ir.sequence'].next_by_code('website.support.ticket.incident')        
        elif sun_tag_ids.name=="Request": 
            print("Request")
            vals['ticket_number'] = self.env['ir.sequence'].next_by_code('website.support.ticket.request')
        else:
            print("else")
...
class SunfireTicketTag(models.Model):
    _name="sunfire.ticket.tag"
    _rec_name="name"
    name=fields.Char(string="Tag Name")
class sunfireTicketChannel(models.Model):
    _name='sunfire.ticket.channel'    
    _rec_name='name'
    name=fields.Char('Channel')
class SunfireTicketAttach(models.Model):
    _name='sunfire.ticket.attach'
    _rec_name='name'
    name=fields.Char(string='Attachment')
    sun_attach=fields.Binary(string="Attachment")
    sun_attach_filename=fields.Char(string="Attachment Filename")
    attach_id=fields.Many2one('website.support.ticket')
class SunfireTicketActioNTaken(models.Model):
    _name='sunfire.actions'
    action_taken=fields.Text('Actions Taken')
    status=fields.Selection([
        ('open', 'Open'),
        ('assigned', 'Assigned'),
        ('in_progress', 'Work in Progress'),
    ...
    action_taken_id=fields.Many2one('website.support.ticket')
    @api.model
    def create(self,vals):
        web_obj=self.env['website.support.ticket'].search([('id','=',vals['action_taken_id'])])
        vals['status']=web_obj.status
        new_id = super(SunfireTicketActioNTaken, self).create(vals)
        return new_id
